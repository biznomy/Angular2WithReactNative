...
gulp.task('compile.jit', ['!copyToNodeModules'], function () {
  return gulp.src(PATHS.tmp + '/workspace/**/*').pipe(gulp.dest(PATHS.app + '/' + APP_NAME));
});
...
gulp.task('compile.copy.source', ['!tmp.clean'], function () {
  return gulp.src(['src/**/*', 'workspace/**/*'], { base: './' }).pipe(gulp.dest(PATHS.tmp));
});
...
gulp.task('compile.ngc.workspace', ['compile.ngc.src'], function (done) {
  exec(ngcExecutable + ' -p ./dist/tmp/workspace', function(e, stdout) {
    if (e) console.log(e);
    if (done) done();
  }).stdout.on('data', function(data) {
    console.log(data);
...
gulp.task('compile.tsc.factories', ['compile.ngc.workspace'], function (done) {
...
gulp.task('compile.finalize', ['compile.tsc.factories'], function (done) {
  exec('cp -rf ./dist/tmp/factories_out/* ./dist/tmp/', function(e, stdout) {
    var del = require('del');
    del([PATHS.tmp + '/factories', PATHS.tmp + '/factories_out', PATHS.tmp + '/workspace', PATHS.tmp + '/src', PATHS.tmp + '/waste'], done);
  })
});
...
gulp.task('compile.aot', ['compile.copy'], function (done) {
  exec('cp ./workspace/index.android.aot.js ./dist/app/InfinityReact/index.android.js', function(e, stdout) {
    exec('cp ./workspace/index.ios.aot.js ./dist/app/InfinityReact/index.ios.js', function(e, stdout) {
      done();
    });
  });
});
